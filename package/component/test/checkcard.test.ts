...
describe("LayCheckCard.vue", () => {
  ...
  test('render avater test', () => {
    ...
    expect(wrapper.find('.layui-checkcard-avatar img').attributes('src')).toBe(IMAGE_URL)
    wrapper.unmount()
  })
  ...
  test('render class、style test', () => {
    ...
    expect(wrapper.find('.layui-checkcard').attributes('style')).toContain(style)
    wrapper.unmount()
  })
  ...
  test('render disabled test', async () => {
    ...
    expect(wrapper.find('.layui-checkcard').classes()).toContain('layui-checkcard-disabled')
    await wrapper.find('.layui-checkcard').trigger('click')
