...
describe("LayPage.vue", () => {
  ...
  test("render vertical test", async () => {
    ...
    expect(wrapper.find('.layui-slider-bar').attributes().style).toContain('height: 50%')
    expect(wrapper.find('.layui-slider-handle').attributes().style).toContain('bottom: 50%')
  })
  test("render range test", async () => {
    ...
    expect(wrapper.find('.layui-slider-bar').attributes().style).toContain('width: 10%')
    await wrapper.setProps({ modelValue:[0, 20] });
    await nextTick()
    expect(wrapper.find('.layui-slider-bar').attributes().style).toContain('width: 20%')
    expect(getDomInfo[0]?.attributes().style).toContain('left: 0%')
  ...
  test("render disabled test", async () => {
    ...
    await nextTick()
    expect(wrapper.find('.layui-slider-bar').attributes().style).toContain('width: 0%')
  ...
  test("render step test", async () => {
    ...
    expect(wrapper.find('.layui-slider-bar').attributes().style).toContain('width: 50%')
    expect(wrapper.find('.layui-slider-handle').attributes().style).toContain('left: 50%')
  })
  ...
  test("render is-follow-mark and mark test", async () => {
    ...
    expect(getMark[getMark.length - 1].attributes().style).toContain('color: #f50')
    expect(wrapper.find('.layui-slider-bar').attributes().style).toContain('width: 37%')
    expect(wrapper.find('.layui-slider-handle').attributes().style).toContain('left: 37%')
  ...
  test("render slot  custom test", async () => {
    ...
    await nextTick()
    expect(wrapper.findAll('.layui-slider-mark-text')[0].text()).toEqual('mark')
  })
