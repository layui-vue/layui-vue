<template>
  <div class="layui-laydate layui-laydate-range">
          ...
          >
            <ol class="scroll" @click="chooseStartTime">
              <li
                ...
                :class="[
                  'num',
                  index == startHms[item.type] ? 'layui-this' : '',
                ]"
              >
    ...
    <PanelFoot @ok="footOnOk" :showNow="false" @now="footOnNow" @clear="footOnClear">
      ...
      至
      <template
        v-if="!isNaN(endHms.hh) && !isNaN(endHms.mm) && !isNaN(endHms.ss)"
      >
        {{
          dayjs()
            .hour(endHms.hh)
...
const footOnOk = () => {
  emits(
    "update:startTime",
    dayjs()
      .hour(startHms.value.hh)
      .minute(startHms.value.mm)
      .second(startHms.value.ss)
  );
  emits(
    "update:endTime",
    dayjs()
      .hour(endHms.value.hh)
      .minute(endHms.value.mm)
      .second(endHms.value.ss)
  );
  datePicker.ok();
};
