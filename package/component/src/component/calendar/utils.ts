...
export const getMonthDay = (date: string | number | Date) => {
  ...
  return Array(days).fill(dayjs).map((_, index) => dayjs(date).set('date', index + 1).format('YYYY-MM-DD'))
}
export const getPrevMonthDay = (date: string | number | Date) => {
  ...
  return Array(firstDay).fill(firstDay).map((_, index) => dayjs(date).subtract(1, 'month').set('date', latstDay - (firstDay - index - 1)).format('YYYY-MM-DD'))
}
export const nextMonthDay = (date: string | number | Date, num: number) => {
  return Array(num).fill(num).map((_, index) => dayjs(date).add(1, 'month').set('date', index + 1).format('YYYY-MM-DD'))
} 