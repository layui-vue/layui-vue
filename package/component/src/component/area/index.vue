<template>
	<div>
		<lay-dropdown ref="areaRef">
			<lay-input :value="cityValue" readonly>
				<!-- 	    <template  #suffix>
          <i :class="'layui-icon layui-icon-close-fill'" @click="qk"></i>
				</template>-->
			</lay-input>
			<template #content>
				<div :class="clz == false ? 'city-picker' : 'city-picker layui-hide'" v-if="!cust">
					<div class="city-picker-tab">
						<span
							:class="cls == true ? 'city-picker-tab-province ' : 'city-picker-tab-province city-picker-active'"
							ref="sz"
							@click="xsf"
						>省份</span>
						<span
							:class="clx == true ? 'city-picker-tab-city city-picker-active' : 'city-picker-tab-city'"
							ref="cs"
							@click="xcs"
						>城市</span>
						<span
							:class="cly == false ? 'city-picker-tab-district' : 'city-picker-tab-district city-picker-active'"
							ref="qx"
						>区县</span>
						<lay-button-group class="city-picker-clear">
							<span type="default" class="btns" size="sm" @click="qk" v-if="simple==true">清空</span>
							<span type="default" class="btns" size="sm" @click="ok" v-if="simple==false">确定</span>
						</lay-button-group>
					</div>
					<div class="city-picker-content">
						<div
							:class="cls == false ? 'city-picker-select city-picker-province city-picker-open' : 'city-picker-select city-picker-province'"
						>
							<div class="city-picker-row">
								<label>A-G</label>
								<p>
									<span @click="sf(item)" v-for="item of ag" :key="item">{{ item }}</span>
								</p>
							</div>
							<div class="city-picker-row">
								<label>H-K</label>
								<p>
									<span @click="sf(item)" v-for="item of hk" :key="item">{{ item }}</span>
								</p>
							</div>
							<div class="city-picker-row">
								<label>L-S</label>
								<p>
									<span @click="sf(item)" v-for="item of ls" :key="item">{{ item }}</span>
								</p>
							</div>
							<div class="city-picker-row">
								<label>T-Z</label>
								<p>
									<span @click="sf(item)" v-for="item of tz" :key="item">{{ item }}</span>
								</p>
							</div>
							<div class="city-picker-row">
								<label>其它</label>
								<p>
									<span @click="sf(item)" v-for="item of qz" :key="item">{{ item }}</span>
								</p>
							</div>
						</div>
						<div
							:class="clx == false ? 'city-picker-select city-picker-city' : 'city-picker-select city-picker-city city-picker-open'"
						>
							<div class="city-picker-row">
								<p>
									<span @click="dcs(item1)" v-for="(item1) in cslist" :key="item1">{{ item1 }}</span>
								</p>
							</div>
						</div>
						<div
							:class="cly == false ? 'city-picker-select city-picker-district' : 'city-picker-select city-picker-district city-picker-open'"
						>
							<div class="city-picker-row">
								<p>
									<span @click="dqx(item2)" v-for="(item2) in qxlist" :key="item2">{{ item2 }}</span>
								</p>
							</div>
						</div>
					</div>
				</div>
				  <div :class="clz == false ? 'city-picker' : 'city-picker layui-hide'" v-else>
                    <div class="city-picker-tab">
                        <span
                            :class="cls == true ? ' ' : ' city-picker-active'"
                            @click="xsf"
                        >{{ title[0] ? title[0] : '' }}</span>
                        <span
                            :class="clx == true ? ' city-picker-active' : ''"
                            @click="xcs"
                        >{{ title[1] ? title[1] : '' }}</span>
                        <span
                            :class="cly == false ? '' : ' city-picker-active'"
                        >{{ title[2] ? title[2] : '' }}</span>
						<lay-button-group class="city-picker-clear">
							<span type="default" class="btns" size="sm" @click="qk" v-if="simple==true">清空</span>
							<span type="default" class="btns" size="sm" @click="ok" v-if="simple==false">确定</span>
						</lay-button-group>
                            ...
                            <div class="city-picker-row">
                                <!-- <ul>
                                    <div  :class="'layui-menu-body-title'" 
                                        @click="sf(item)"
                                        v-for="item of Object.keys(json)"
                                        :key="item"
                                    >{{ item }}</div>
                                </ul>-->
                                <p>
                                    <span
                                        @click="dyj(item)"
                                        v-for="item of Object.keys(data)"
                                        :key="item"
                                    >{{ item }}</span>
                                </p>
                            </div>
...
<!--                     <div class="footer-btns">
                        <lay-button-group>
                            <lay-button type="default" class="btns" size="sm" @click="qk">清空</lay-button>
                            <lay-button type="default" class="btns" size="sm" @click="ok">确定</lay-button>
                        </lay-button-group>
                    </div> -->
                </div>
			</template>
...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		const ag = reactive(['安徽省', '北京市', '重庆市', '福建省', '甘肃省', '广东省', '广西壮族自治区', '贵州省'])
		const hk = reactive(['海南省', '河北省', '黑龙江省', '河南省', '湖北省', '湖南省', '江苏省', '江西省', '吉林省'])
		const ls = reactive(['辽宁省', '内蒙古自治区', '宁夏回族自治区', '青海省', '山东省', '上海市', '山西省', '陕西省', '四川省'])
		const tz = reactive(['天津市', '新疆维吾尔自治区', '西藏自治区', '云南省', '浙江省'])
		const qz = reactive(['香港', '澳门', '台湾'])
		...
		const json = JSON.stringify({
			...
			"广东省": {
				...
				"东莞市": ["东城街道", "南城街道", "万江街道", "莞城街道", "石碣镇", "石龙镇", "茶山镇", "石排镇", "企石镇", "横沥镇", "桥头镇", "谢岗镇", "东坑镇", "常平镇", "寮步镇", "樟木头镇", "大朗镇", "黄江镇", "清溪镇", "塘厦镇", "凤岗镇", "大岭山镇", "长安镇", "虎门镇", "厚街镇", "沙田镇", "道滘镇", "洪梅镇", "麻涌镇", "望牛墩镇", "中堂镇", "高埗镇", "松山湖", "东莞港", "东莞生态园"],
				"中山市": ["石岐街道", "东区街道", "中山港街道", "西区街道", "南区街道", "五桂山街道", "小榄镇", "黄圃镇", "民众镇", "东凤镇", "东升镇", "古镇镇", "沙溪镇", "坦洲镇", "港口镇", "三角镇", "横栏镇", "南头镇", "阜沙镇", "南朗镇", "三乡镇", "板芙镇", "大涌镇", "神湾镇"],
				"潮州市": ["湘桥区", "潮安区", "饶平县"],
			...
			"台湾": {
				...
				"新北市": ["板桥区", "新庄区", "中和区", "永和区", "土城区", "树林区", "三峡区", "莺歌区", "三重区", "芦洲区", "五股区", "泰山区", "林口区", "八里区", "淡水区", "三芝区", "石门区", "金山区", "万里区", "汐止区", "瑞芳区", "贡寮区", "平溪区", "双溪区", "新店区", "深坑区", "石碇区", "坪林区", "乌来区"],
				"桃园市": ["桃园区", "中坜区", "平镇区", "八德区", "杨梅区", "芦竹区", "大溪区", "龙潭区", "龟山区", "大园区", "观音区", "新屋区", "复兴区"],
				"台中市": ["中区", "东区", "南区", "西区", "北区", "北屯区", "西屯区", "南屯区", "太平区", "大里区", "雾峰区", "乌日区", "丰原区", "后里区", "石冈区", "东势区", "新社区", "潭子区", "大雅区", "神冈区", "大肚区", "沙鹿区", "龙井区", "梧栖区", "清水区", "大甲区", "外埔区", "大安区", "和平区"],
				...
				"基隆市": ["仁爱区", "中正区", "信义区", "中山区", "安乐区", "暖暖区", "七堵区"],
				"新竹市": ["东区", "北区", "香山区"],
				"嘉义市": ["东区", "西区"],
				"新竹县": ["竹北市", "竹东镇", "新埔镇", "关西镇", "湖口乡", "新丰乡", "峨眉乡", "宝山乡", "北埔乡", "芎林乡", "横山乡", "尖石乡", "五峰乡"],
				...
				"云林县": ["斗六市", "斗南镇", "虎尾镇", "西螺镇", "土库镇", "北港镇", "林内乡", "古坑乡", "大埤乡", "莿桐乡", "褒忠乡", "二仑乡", "仑背乡", "麦寮乡", "台西乡", "东势乡", "元长乡", "四湖乡", "口湖乡", "水林乡"],
				"嘉义县": ["太保市", "朴子市", "布袋镇", "大林镇", "民雄乡", "溪口乡", "新港乡", "六脚乡", "东石乡", "义竹乡", "鹿草乡", "水上乡", "中埔乡", "竹崎乡", "梅山乡", "番路乡", "大埔乡", "阿里山乡"],
				"屏东县": ["屏东市", "潮州镇", "东港镇", "恒春镇", "万丹乡", "长治乡", "麟洛乡", "九如乡", "里港乡", "盐埔乡", "高树乡", "万峦乡", "内埔乡", "竹田乡", "新埤乡", "枋寮乡", "新园乡", "崁顶乡", "林边乡", "南州乡", "佳冬乡", "琉球乡", "车城乡", "满州乡", "枋山乡", "雾台乡", "玛家乡", "泰武乡", "来义乡", "春日乡", "狮子乡", "牡丹乡", "三地门乡"],
				"宜兰县": ["宜兰市", "头城镇", "罗东镇", "苏澳镇", "礁溪乡", "壮围乡", "员山乡", "冬山乡", "五结乡", "三星乡", "大同乡", "南澳乡"],
				"花莲县": ["花莲市", "凤林镇", "玉里镇", "新城乡", "吉安乡", "寿丰乡", "光复乡", "丰滨乡", "瑞穗乡", "富里乡", "秀林乡", "万荣乡", "卓溪乡"],
				"台东县": ["台东市", "成功镇", "关山镇", "长滨乡", "池上乡", "东河乡", "鹿野乡", "卑南乡", "大武乡", "绿岛乡", "太麻里乡", "海端乡", "延平乡", "金峰乡", "达仁乡", "兰屿乡"],
				"澎湖县": ["马公市", "湖西乡", "白沙乡", "西屿乡", "望安乡", "七美乡"],
				"金门县": ["金城镇", "金湖镇", "金沙镇", "金宁乡", "烈屿乡", "乌丘乡"],
				"连江县": ["南竿乡", "北竿乡", "莒光乡", "东引乡"]
			},
			"澳门": {
				"堂区": ["花地玛堂区", "圣安多尼堂区", "大堂区", "望德堂区", "风顺堂区", "嘉模堂区", "圣方济各堂区", "路氹城"
				]
			},
			"香港": {
				"香港岛": ["中西区", "湾仔区", "东区", "南区"],
		...
		let qxlist = ref([''])
		// 点击具体省份
		const sf = function (sf: string) {
			xzsf.value = sf
			if (['北京市', '重庆市', '上海市', '天津市'].includes(sf)) {
				cls.value = true
				dcs('市辖区')
			} else {
				cls.value = true
			...
			let cssz = Object.keys(JSON.parse(json)[sf])
			cslist.value = cssz
			cityValue.value = sf

		}
		// 点击具体城市
		const dcs = function (cs: string) {
			cly.value = true
			clx.value = false
			xzcs.value = cs
			let qxsz
			if (xzsf.value == '重庆市') {
				qxsz = [...JSON.parse(json)[xzsf.value][cs], ...JSON.parse(json)[xzsf.value]['县']]
			} else {
				qxsz = JSON.parse(json)[xzsf.value][cs]
			}
			qxlist.value = qxsz
			if (['北京市', '重庆市', '上海市', '天津市'].includes(xzsf.value)) {
				cityValue.value = xzsf.value

			} else {
				cityValue.value = xzsf.value + cs

			}

		}
		// 点击具体区县
		const dqx = function (qx: string) {
			xzqx.value = qx
			if (['北京市', '重庆市', '上海市', '天津市'].includes(xzsf.value)) {
				cityValue.value = xzsf.value + qx
				emit("update:modelValue", cityValue.value);
			} else {
				cityValue.value = xzsf.value + xzcs.value + qx
				emit("update:modelValue", cityValue.value);
			}
			   if(props.simple == true){  //关闭下拉框
                   ok()
               } 
		...
		const shijian = reactive({
            // 点击第一级具体选项
            dyj: function (sf: string) {
                xzsf.value = sf
                cls.value = true
                clx.value = true
                cly.value = false
                let cssz = Object.keys(props.data[sf])
                cslist.value = cssz
                if (props.bool == false) {
                    cityValue.value = sf
                } else if (props.bool == true) {
                    cityValue.value = ''
                }
            },
            // 点击第二级具体选项
            drj: function (cs: string) {
                cly.value = true
                clx.value = false
                xzcs.value = cs
                let qxsz = props.data[xzsf.value][cs]
                qxlist.value = qxsz
                if (props.bool == false) {
                    cityValue.value = xzsf.value + cs
                } else if (props.bool == true) {
        ...
        const todata = toRefs(shijian)
		return {
			...todata,cityValue, sf, ag, xzsf, sz, cs, qx, qk, clx, cls, dcs, clz, cly, cslist, xsf, hk, ls, tz, qz, dqx, qxlist, xcs, ok, areaRef
		}
	}
...
.city-picker-tab > span:hover {
	background: #5d6778;
	transition: all 0.3s;
}
.city-picker-tab > span.city-picker-active {
	background: #fff;
	color: #5a687e;
}
.city-picker-clear {
...
.city-picker-select .city-picker-row {
	line-height: 2;
	clear: both;
	padding: 3px 0;
	margin: 0;
...
.city-picker-select .city-picker-row label {
	white-space: nowrap;
	min-width: 50px;
	color: #888;
}
.city-picker-select span {
	display: inline-block;
	padding: 0 10px;
	outline: 0;
	text-decoration: none;
...
.city-picker-select span:hover {
	background: #eaf1f8;
	border-radius: 2px;
	color: #477fdd;
}
.city-picker-select span.city-picker-action {
